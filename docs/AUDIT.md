...
두 레퍼런스 모두 **"우리가 했던 일의 실제 결과물"** 을 전면에 내세운다. 그로스웨이브는 현재 PLACEHOLDER 데이터를 노출 중이라, 레퍼런스 대비 신뢰의 물질적 증거(social proof)가 가장 약하다. 카피 톤은 오히려 절제되어 있어 차별점이다.
...
| 파일 | verbatim | 비고 |
|------|----------|------|
| [`src/data/ceo-message.ts`](../src/data/ceo-message.ts) line 5 | `"그래서 그로스웨이브를 새로 시작했습니다. 6개 직군의 광고 규정을 직접 검토하고…"` | 대표 인사말 본문 |
...
| 항목 | 위치 | 표현 |
|------|------|------|
| Since 2026 표기 | [`src/components/sections/AESlogan.tsx`](../src/components/sections/AESlogan.tsx) line 17 | `Growth Wave / Since 2026` |
...
**문제**: 회사 창립은 2026년인데 "8년 누적"을 내세우면, 대표 개인 경력 8년(2018~)과 회사 운영 기간을 혼동하게 된다. CEO 타임라인은 2023년 T사 입사부터 시작해 2026년 그로스웨이브 정식 운영이므로, 방문자가 "2026년 창업인데 8년은 어떻게?"라는 의문을 가질 수 있다.
...
| **현재** | `"규정에 걸리지 않습니다."` |
| **문제** | 절대적 약속처럼 들린다. 광고 규정은 해석과 적용이 유동적이며, 행정처분·징계 주체는 각 협회와 기관이다. 그로스웨이브가 "걸리지 않음"을 보장할 수 없는 영역. 향후 위반 사례 발생 시 법적 분쟁 소지. |
